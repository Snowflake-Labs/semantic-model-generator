...
from shared_utils import SnowflakeStage, stage_exists
...
with st.form(key="storage", border=False):
    ...
    # with col1:
    stage_database = st.text_input(
        "Database", key="stage_database_key", value="SNOWFLAKE_SEMANTIC_CONTEXT"
    )
    # with col2:
...
if save:
    ...
    if not (len(stage_database) > 0 and len(stage_schema) > 0 and len(stage_name) > 0):
